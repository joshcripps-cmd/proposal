...
export default function AdminDashboard() {
  ...
  const [proposals, setProposals] = useState([]); // starts empty — no mock data
  const [yachts, setYachts] = useState([]);        // starts empty — populated by XLSX
  ...
  const handleSaveProposal = (form) => {
    if (form.yachts_detail) {
      setYachts(prev => {
        const updated = [...prev];
        form.yachts_detail.forEach(fy => {
          const idx = updated.findIndex(y => y.name === fy.name);
          if (idx >= 0) updated[idx] = { ...updated[idx], discount: fy.discount || 0 };
        });
        return updated;
      });
    }
    if (editingProposal) {
      setProposals(prev => prev.map(p => p.id === form.id ? { ...p, ...form } : p));
    } else {
      setProposals(prev => [{ ...form, id: `prop_${Date.now()}`, views: 0, unique_viewers: 0, shortlisted: [], created_at: new Date().toISOString().split("T")[0] }, ...prev]);
    }
    setEditingProposal(null);
    setPage("proposals");
  };

  const handleXlsxUpload = (parsed) => {
    setYachts(prev => {
      const merged = [...prev];
      parsed.forEach(y => {
        const idx = merged.findIndex(m => m.name.toLowerCase() === y.name.toLowerCase());
